...
public class DayWorkFragment extends TabViewFragment {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		btnReCalc = (Button) activity.findViewById(R.id.day_work_search_button);
		txtQueryDate = (TextView) activity
				.findViewById(R.id.day_work_query_date);
		listView = (ListView) activity
				.findViewById(R.id.house_day_works_listview);
		m_Access = new BasicAccess(activity);
		btnPrev = (Button) activity.findViewById(R.id.day_work_prev_day);
		btnNext = (Button) activity.findViewById(R.id.day_work_next_day);
						...
						.setPositiveButton("是",
								new DialogInterface.OnClickListener() {
									...
									public void onClick(DialogInterface dialog,
										...
										try {
											m_Access.Visit(
													DayWorkDetailAccess.class)
													.ReCalcOnline(queryDateStr,
															getCurrentGroupId());
											RunQueryServices();
										} catch (Exception e) {
		...
		listView.setOnCreateContextMenuListener(new View.OnCreateContextMenuListener() {
			...
			public void onCreateContextMenu(ContextMenu menu, View v,
				...
				if (minfo.position >= houseWorkList.size())
					return;
				...
				if (selectedDayWorkHouse.PeopleComingList != null) {
					for (int i = 0; i < selectedDayWorkHouse.PeopleComingList
							.size(); i++) {
						PeopleComing pc = selectedDayWorkHouse.PeopleComingList
								.get(i);
				...
				if (selectedDayWorkHouse.Works != null) {
					for (int i = 0; i < selectedDayWorkHouse.Works.size(); i++) {
						menu.add(11, i, pos,
								selectedDayWorkHouse.Works.get(i).MemberName
										+ "工作");
						pos++;
					}
	...
	private void GotoEditDayWorks(int arg2) {
		...
		String h = null;
		if (arg2 > -1)
			h = houseWorkList.get(arg2).HouseID;
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == android.app.Activity.RESULT_OK) {
			if (requestCode == OpenEditPeopleWorking) {
				if (selectedDayWorkHouse == null)
					return;
				selectedDayWorkHouse.PeopleComingList = this.m_Access.Visit(
						PeopleComingAccess.class).GetWorking(
						selectedDayWorkHouse.HouseID, queryDateStr,
						getCurrentGroupId());
				TextView editText = (TextView) selectListViewItem
						.findViewById(R.id.item_house_working_people_coming_1);
				if (editText != null) {
					editText.setText(Html.fromHtml(listItemAdapter
							.FormatPeopleComing(selectedDayWorkHouse)));
				} else {
	...
	private void RunQuery(int absDay) {
		...
		try {
			houseList = m_Access.Visit(HouseAccess.class).Query(false,
					getApp().getCurrentGroupID());
			QueryAskForLeave();
			QueryTask();
			RunQueryServices();
	...
	private void QueryAskForLeave() {
		askList = null;
		if (Utility.UseLocal) {
			askList = m_Access.Visit(AskForLeaveAccess.class).Query(false,
					queryDateStr, getApp().getCurrentGroupID());
			willBackList = m_Access.Visit(AskForLeaveAccess.class)
					.QueryNext2DayBack(queryDateStr,
							getApp().getCurrentGroupID());
			overDateList = m_Access.Visit(AskForLeaveAccess.class)
					.QueryOverDayUnBack(queryDateStr,
							getApp().getCurrentGroupID());

		} else {
			ws.visitServices(
					"AskForLeave_QueryByDayJson",
					new String[] { "day", "groupID" },
					new String[] { queryDateStr,
							String.valueOf(getApp().getCurrentGroupID()) },
					QueryAskForLeave);
		}
	...
	private void QueryTask() {
		taskList = null;
		if (Utility.UseLocal) {
			taskList = m_Access.Visit(TaskAccess.class).GetByDate(queryDateStr,
					true, getApp().getCurrentGroupID());
			// FillTable();
		} else {
			ws.visitServices(
					"Task_GetDayTasksJson",
					new String[] { "day", "groupID" },
					new String[] { queryDateStr,
							String.valueOf(getApp().getCurrentGroupID()) },
					QueryTask);
	...
	private void RunQueryServices() throws Exception {
		houseWorkList = null;
		if (Utility.UseLocal) {
			houseWorkList = m_Access
					.Visit(DayWorkDetailAccess.class)
					.QueryHouseWorks(queryDateStr, getApp().getCurrentGroupID());
			FillTable();

		} else {

			// String method = queryMethod == null
			// ?"DayWork_QueryHouseWorksJson": queryMethod;
			// ws.visitServices(method, new String[]{"day","groupID"},new
			// String[]{
			// queryDateStr,String.valueOf(getApp().getCurrentGroupID())},
			// QueryHouseWork);
		}

	}

	final int QueryHouseWork = 1, ChangeHouse = 2, QueryAskForLeave = 3,
			QueryTask = 4, OpenEditPeopleWorking = 5, QuickEditWork = 6;
	...
	private void InitPopDialog() {
		if (memberWorkDialog == null) {
			memberWorkDialog = new PopMemberDayWorkActivity(this.getActivity(),
					new IPopMemberWorkCallback() {

						@Override
						public void Callback(View v, DayWorkDetail DayWorkDetail) {
							try {
								m_Access.Visit(DayWorkDetailAccess.class)
										.Update(DayWorkDetail);
							} catch (Exception e) {
							...
							m_Access.Close(true);
							TextView editText = (TextView) selectListViewItem
									.findViewById(R.id.item_house_working_house_member1);
							if (editText != null) {
								editText.setText(Html.fromHtml(DayWorkListAdapter
										.FormatHouseMemberWork(selectedDayWorkHouse)));
							} else {
	...
	private void FillTable() {
		if (houseWorkList == null || askList == null || taskList == null)
			return;
		// houseList = new LinkedList<House>();
		boolean needInit = true;
		/*
		 * for (DayWorkHouse hdw:houseWorkList) { if
	...
	private void FormartListViewAdapter() {
		...
		// java.util.List<HashMap<String, Object>> listItem = new
		// ArrayList<HashMap<String, Object>>();
		// clear empty
		for (int i = houseWorkList.size() - 1; i > -1; i--) {
			DayWorkHouse hdw = houseWorkList.get(i);
			if ((hdw.PeopleComingList == null || hdw.PeopleComingList.size() == 0)
					&& (hdw.Works == null || hdw.Works.size() == 0))
				houseWorkList.remove(i);
		}
		...
		listItemAdapter = new DayWorkListAdapter(getActivity(), houseWorkList,
				taskList, askList, willBackList, overDateList);
	...
	// {
	// int peopleSize = DayWorkHouse.PeopleComingList == null ? 0:
	// DayWorkHouse.PeopleComingList.size();
	// String peopleStr = "";
	// for (int i=0;i < peopleSize;)
	// {
	// PeopleComing pc = DayWorkHouse.PeopleComingList.get(i);
	// peopleStr += "["+pc.MemberName +pc.ComeFrom + pc.Relation +"] "+
	// pc.DayResult;
	// i++;
	...
	// if (daywork.Remark != null && daywork.Remark != "") strItem +=
	// daywork.Remark;
	// if(daywork.IsHouseChanged) strItem="<font color='#5c6063'>"+ strItem
	// +"</font>";
	// workStr += strItem +"&nbsp;&nbsp;";
	// }
	//
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case 10:
			...
			if (pc == null)
				Log.e("error", "people comeing is null");
		...
		case 11:
			...
			selectDayWork = (DayWorkDetail) selectedDayWorkHouse.Works
					.get(editMemberIndex);
		...
		case 12:// 搬家
			...
			if (chooseHouseDialog == null) {
				chooseHouseDialog = new AlertDialog.Builder(getActivity())
						...
						.setSingleChoiceItems(houseArr, selectedHouseIndex,
								new DialogInterface.OnClickListener() {

									public void onClick(DialogInterface dialog,
											int which) {
										newHouse = houseList.get(which);
										// ws.visitServices("DayWork_ChangeHouseJson",
										// new
										// String[]{"memberId","oldHourse","newHourse","day"},
										// new String[]{
										...
										try {
											arr = m_Access
													.Visit(DayWorkDetailAccess.class)
													.ChangeHouse(
															selectDayWork.MemberID,
															selectedDayWorkHouse.HouseID,
															newHouse.ID,
															queryDateStr,
															getCurrentGroupId());
										} catch (Exception e) {
	...
	// selectListViewItem.findViewById(R.id.item_house_working_house_member1);
	// if (editText != null)
	// {
